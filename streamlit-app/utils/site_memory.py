...
def list_memory_items(
    *,
    ...
    rows = (adapter or LocalJsonStorageAdapter()).list_records(**filters)
    return [MemoryItem.from_dict(row) for row in rows]
...
def search_memory(
    ...
    *,
    ...
    if project_id:
        filters["project_id"] = project_id
    rows = (adapter or LocalJsonStorageAdapter()).search_records(query, **filters)
    return [MemoryItem.from_dict(row) for row in rows]
